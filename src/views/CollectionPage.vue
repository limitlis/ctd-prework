...
async function getPagedResults() {
    ...
    if (search.value?.length) {
        ...
        const searchResult = await api
            .get<
                ApiResponseData<ESSearchResult[]>
            >(`${route.params.collection}${search.value?.length ? '/search' : ''}?${queryParams.join('&')}`)
            .catch((err) => {
                error.value = err;
            });
        if (searchResult?.status === 200) {
            ...
            if (searchResult.data?.data?.length) {
                ...
                const result = await api
                    .get<ApiResponseData<T[]>>(`${route.params.collection}?ids=${ids.join()}`)
                    .catch((err) => {
                        error.value = err;
                    });
                if (result?.status === 200) {
                    ...
                    router.replace({
                        path: route.path,
                        query: { page: currentPage.value, search: search.value },
                    });
                    loading.value = false;
                    return;
                }
        ...
        const result = await api
            .get<ApiResponseData<T[]>>(`${route.params.collection}?${queryParams.join('&')}`)
            .catch((err) => {
                error.value = err;
            });
        if (result?.status === 200) {
    ...
    <main>
        <div class="w-full border-b pb-4">
            <form @submit.prevent="doSearch">
                <div class="relative flex items-center">
                    ...
                    <button
                        class="rounded-md py-2 px-4 border text-center text-sm transition-all pointer-cursor hover:shadow-lg focus:bg-accent focus:shadow-none active:bg-accent hover:bg-accent active:shadow-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none ml-2"
                        type="submit"
                    >
                        Search
                    </button>
        ...
        <template v-if="!loading && results.length > 0">
            <div class="grid grid-cols-12 gap-6 py-8">
                <CollectionCard
                    v-if="['artworks', 'exhibitions'].includes(route.params.collection as string)"
