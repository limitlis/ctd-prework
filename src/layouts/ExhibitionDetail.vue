...
async function getFeaturedArtworks() {
    ...
    const result = await api
        .get<ApiResponseData<Artwork[]>>(`artworks?ids=${item.artwork_ids.join()}`)
        .catch((err) => {
            error.value = err;
        });
...
async function getFeaturedArtists() {
    const result = await api
        .get<ApiResponseData<Artist[]>>(`artists?ids=${item.artist_ids.join()}`)
        .catch((err) => {
            error.value = err;
        });
    if (result?.status === 200) {
...
<div v-if="item" class="space-y-5">
    ...
    <template v-if="item.artwork_ids?.length">
